...
- ~~bundle-gateway.zstd를 기본 번들로 교체~~ ✓ (Release v3 번들로 교체)
- ~~gen_catalog_docs에서 PartialStub을 Available 섹션에 포함~~ → 설계 완료 (구현 예정)
- ~~E2E 테스트: 실제 PartialStub API에서 available operation 호출~~ → 설계 완료 (Gateway AJAX Available로 전환)

---

## 2026-04-05: PartialStub 마무리 설계 + 번들 배포 파이프라인

### 배경

PartialStub + CI 파이프라인 후속으로 남은 3개 Task(번들 교체, 문서 분류, E2E)에 대한 설계 세션. 번들 배포 전략에 대한 아키텍처 결정도 함께 진행.

### 완료

- 설계 스펙: `docs/specs/2026-04-05-partial-stub-finalization-design.md`
- Phase 1.1 § 5 섹션 로드맵 추가
- 번들 배포 파이프라인 인프라 구축:
  - `build.rs` 3단계 fallback (로컬 → `BUNDLE_DOWNLOAD_URL` env → placeholder)
  - `.github/workflows/release.yml` 신규 (4 플랫폼 크로스 빌드: Linux musl, macOS x64/arm, Windows x64)
  - `Cargo.toml include` 필드 (crates.io publish 시 `data/bundle.zstd` 포함)
  - `scripts/publish.sh` 신규 (cargo publish 전 번들 다운로드 래퍼)
- `data/bundle.zstd`를 Release `bundle-2026-04-04-4` v3 번들로 교체

### 핵심 결정

- **Option A' (임베드 번들 유지)**: No-embed + 런타임 자동 다운로드 방식은 MCP stdio 컨텍스트에서 구조적 실패. Claude Desktop이 stderr를 log 파일로 리다이렉트하므로 initialize timeout 시 "도구 없음"만 노출됨 (MCP Transport 스펙상 클라이언트가 stderr를 버릴 수 있음: `MAY capture`)
- **3-tier 번들 공급**: 로컬 개발자(`make update-bundle` 헬퍼) + 바이너리 CI(`BUNDLE_DOWNLOAD_URL` 주입) + crates.io publish(`scripts/publish.sh` + `include` 필드)
- **schema v3 → v4 마이그레이션 계획**: `ApiSpec.missing_operations: Vec<String>` 추가. PartialStub API에서 실패 operation 이름(summary/op_name)을 번들에 저장 → 카탈로그 문서에서 "누락" 목록 표시 가능
- **Task 3 범위 전환**: 원래 "PartialStub API 호출 E2E"였으나 PartialStub 0건 발생으로 불가. Gateway AJAX Available API 수동 스모크 테스트로 전환

### 전문가 교차검증 결과

- **deployment-engineer**: No-embed 자동 다운로드 권장 (DX/복잡도 감소)
- **backend-architect**: A 유지 권장 (MCP 신뢰성 + 배포 원자성 + schema drift 방지)
- **Codex**: A 유지 확정 (MCP Transport 스펙 직접 확인 + Cargo `include` 필드 `.gitignore` override 확인)
- **결론**: 2:1로 A 유지. Codex가 MCP stdio 실패 시나리오를 스펙 기반으로 증명

### PartialStub 실제 발생률 조사

- 4/4 CI 실행 결과: `failed_ops.json` 비어있음 → **PartialStub 0건**
- `fetch_gateway_spec` 감지 로직 자체는 버그 없음 (operation별 독립 AJAX 요청)
- 원인: CI 환경 안정성 + rate limiting 적절 + timeout 30초 넉넉 → 부분 실패 미발생
- feature는 timeout/간헐 오류 방어망으로 유지 (3개월 후 재평가)

### Task 3 E2E 대상 5개 API (사용자 이용신청 완료)

- 15059468 (기상청 중기예보), 15012690 (한국천문연구원 특일), 15073855 (에어코리아 대기오염통계), 15000415 (기상청 기상특보), 15134735 (국토교통부 건축물대장)

### 다음 작업

- `Makefile update-bundle` 헬퍼 추가
- schema v4 마이그레이션: `ApiSpec.missing_operations` + `build_bundle.rs` 수집 로직
- `gen_catalog_docs` PartialStub 분류 전환 + 상태/누락 컬럼 추가
- CI 재빌드 트리거 → v4 번들 Release 생성
- `tests/integration/e2e_gateway_smoke.rs` 작성 + 수동 실행

---

## 2026-04-06: PartialStub 마무리 구현 (Task 1-8, 10)

### 완료

플랜 `docs/plans/2026-04-05-partial-stub-finalization.md`의 Task 1-8, 10 구현 (13 커밋):

- **Task 1** (7cc6123): orphan `data/bundle-gateway.zstd` 삭제 + `bundle-ci.yml --latest` 제거 + `PREV_TAG` null 처리
- **Task 2** (0ea28fb): schema v3 → v4 — `ApiSpec.missing_operations: Vec<String>` 필드 맨 마지막 추가 (postcard varint 순서 보존), `html_parser`/`swagger` 빌더 기본값 주입, `test_load_embedded_bundle` 과도기 허용
- **Task 3** (f271a49): `fetch_gateway_spec` 에서 `failed_ops.op_name → spec.missing_operations` 매핑 (빈 문자열 필터) + `merge_operations`에서 retry 복구 op 제거 (정확 일치 매칭)
- **Task 4** (932e1c6): `src/bin/verify_bundle.rs` 신규 — `postcard::take_from_bytes::<BundleMetadata>` peek 방식으로 schema_version 비교 (struct 전체 호환성 의존 제거)
- **Task 5** (9a1c7e9): `release.yml`에 번들 schema 검증 gate 추가 (다운로드 + verify-bundle release 빌드 + 바이너리 직접 호출)
- **Task 6** (1281c62): `bundle.rs` embedded 번들 로드 실패 시 사용자 친화적 panic 메시지
- **Task 7** (5e72bc5): `gen_catalog_docs` — `SpecStatus.is_callable()` 기반 Available 섹션 필터링, "상태"(✓/⚠️ 부분) + "누락" 컬럼 추가, "완전 N + 부분 M" 통계
- **Task 8** (caf651c): `caller.rs` XML 응답 파싱 분기 + `parse_xml_body` (quick-xml Reader 이벤트 custom parser), base_url trailing slash 방어
- **Task 10** (c6a3c2e): `tests/integration/e2e_gateway_smoke.rs` — 5개 Gateway AJAX Available API 스모크 테스트 (DATA_GO_KR_API_KEY 기반 수동 실행, `#[ignore]`)

### 핵심 결정

- **Schema v4 마이그레이션은 파괴적 변경**: postcard는 필드 추가 시 trailing bytes로 역직렬화 실패. v3 번들은 v4 바이너리 불호환. 3중 방어(release CI gate, bundle.rs override chain, BUNDLE Lazy panic)로 잘못된 조합 차단
- **verify-bundle은 `BundleMetadata` peek**: `Bundle` 전체 deserialize는 schema mismatch 시 postcard 에러로 먼저 실패 → schema_version 비교 dead code 문제. `postcard::take_from_bytes::<BundleMetadata>` 로 첫 필드만 peek (types.rs에 주석 강화)
- **XML 파서 자체 구현**: `quick-xml::de::from_str` serde 모드는 `$text` 래퍼 + 1개/N개 배열 타입 불일치 문제. custom Reader 이벤트 파서로 flat tag → JSON Object 매핑 (`$text` 키로 mixed content 보존, `Event::CData` 처리, self-closing root 태그 지원)
- **merge_operations 매칭 전략**: 한국어 공통 어근("조회", "발표", "목록") false-positive 우려로 substring 폐기 → **정확 일치만**. stale missing_operations는 다음 수집 라운드에서 재계산되어 해소
- **is_callable() 기반 호출 분류**: `SpecStatus::Available | PartialStub` 두 variant를 호출 가능으로 묶음. cli/call.rs, mcp/tools.rs는 이미 이 분기 구현. `mcp/server.rs` description에 "Available 또는 PartialStub" 명시

### Eval 결과 (Round 1 + 2 + Codex 교차검증)

- **Round 1 BLOCK 4건 수정**: parse_xml_body mixed content/CDATA 소실, merge_operations 양방향 substring false-positive, Windows `.exe` 확장자 누락, BUNDLE_DOWNLOAD_URL env 누출 위험
- **Round 2 WARNING 다수 반영**: BundleMetadata 호환성 전제 주석 강화, test_load_embedded_bundle override 경로 의존성 제거, mcp/server.rs description 업데이트, curl `--retry` 추가, mixed content 테스트 추가
- **Codex 교차검증 통과**: W1(merge_operations union 방어 필터), S1(Event::Text leading/trailing trim) 반영. postcard::take_from_bytes 전제 확인, XML namespace 무해, verify-bundle matrix 구조 정당성 검증

### 테스트

- 단위/통합 테스트: 210 PASS (신규 9개: CDATA 보존, self-closing root, mixed content, Value 기반 extract, CDATA 처리 등)
- E2E 테스트: 단위 테스트 PASS, `#[ignore]` 수동 실행은 Task 11로 이관

### Task 9 완료 (2026-04-06)

- CI 트리거 (`gh workflow run bundle-ci.yml`) → `bundle-2026-04-05-6` (v4 schema) 릴리즈 생성
- `Makefile` 신규: `make update-bundle` (최신 릴리즈 다운로드 + verify-bundle guard), `make verify-bundle-local`
- `scripts/publish.sh` 강화: verify-bundle guard, 다운로드 실패 처리, jq null 방지, CI 비대화형 read 대응, dirty src 방어
- `docs/api-catalog/` v4 번들 기반 재생성: 12135 API, 5547 specs, 406 기관
- Eval 3라운드 (deployment-engineer + architect-reviewer + Codex): BLOCK 3 + WARNING 7 모두 해소

### 다음 작업

- Task 11 (사용자): `DATA_GO_KR_API_KEY` 환경변수로 E2E 스모크 수동 실행 + 결과 기록
