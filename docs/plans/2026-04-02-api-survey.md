...
**Goal:** data.go.kr 12K+ API의 openapi.do 페이지를 전수 스캔하여, Swagger/HTML/외부URL 등 실제 패턴 분포를 조사하는 `survey` 바이너리를 만든다.

**Architecture:** `build_bundle.rs`와 동일하게 카탈로그를 수집한 후, 각 API의 openapi.do 페이지를 가져와서 Swagger/HTML/endpoint 패턴을 모두 탐지한다. 결과를 JSON으로 저장하고, 요약 보고서를 stderr에 출력한다. `selectApiDetailFunction.do` 호출은 하지 않는다 (1차 조사는 openapi.do 페이지만).

**Tech Stack:** Rust, tokio, reqwest, serde_json, futures (기존 의존성만 사용)

---

## Task 1: `SurveyResult` 구조체와 직렬화 테스트

**Files:**
- Create: `src/bin/survey.rs`

**Step 1: `SurveyResult` 구조체 정의와 JSON 직렬화 테스트 작성**

```rust
// src/bin/survey.rs 최상단
...
Run: `cargo test --bin survey`
Expected: 2 tests PASS

**Step 3: 커밋**

```bash
git add src/bin/survey.rs Cargo.toml
git commit -m "feat(survey): SurveyResult 구조체 + JSON 직렬화"
```

---

## Task 2: `survey_single_api()` — 단일 API 페이지 조사 함수

기존 `build_bundle.rs`의 `fetch_single_spec()`과 달리, 성공/실패 여부와 관계없이 **모든 신호를 기록**한다.

**Files:**
- Modify: `src/bin/survey.rs`

**Step 1: 테스트 작성 — HTML 파싱 탐지 로직 단위 테스트**

`survey_single_api()`는 HTTP를 호출하므로 단위 테스트가 어렵다. 대신 페이지 분석 로직을 순수 함수 `analyze_page()`로 분리하고, 그 함수를 테스트한다.
...
fn analyze_page(html: &str) -> PageAnalysis {
    ...
    let (swagger_ops_count, swagger_error) = if let Some(ref json) = swagger_json {
        // 임의 list_id로 파싱 시도 — operation 수만 세면 됨
...
fn detect_anomalies(
    ...
    // 5. 리다이렉트 안내 (meta refresh, JavaScript redirect)
    ...
    // 9. swaggerJson이 backtick이 아닌 다른 형태로 존재
    ...
    // 10. 페이지 본문이 비정상적으로 짧음 (에러 페이지 가능성)
    if html.len() < 1000 {
...
mod tests {
    ...
    fn test_analyze_page_html_only() {
        ...
        // swaggerJson 변수 자체가 없으므로 "swagger_json_var_but_unparsed" anomaly 없음
    ...
    fn test_detect_anomalies_unparsed_swagger_var() {
        // swaggerJson 변수는 있지만 backtick이 아닌 다른 형태
        let html = r#"<html><body><script>var swaggerJson = JSON.parse(data);</script></body></html>"#;
        let anomalies = detect_anomalies(html, &None, &None, false);
...
```bash
git add src/bin/survey.rs
git commit -m "feat(survey): analyze_page + detect_anomalies 순수 함수"
```

---

## Task 3: `extract_domain()` 헬퍼 + endpoint URL 분석
...
fn test_extract_domain() {
    assert_eq!(extract_domain("https://apis.data.go.kr/1360000/Weather"), "apis.data.go.kr");
    assert_eq!(extract_domain("https://api.odcloud.kr/api/test"), "api.odcloud.kr");
    assert_eq!(extract_domain("https://apihub.kma.go.kr/api"), "apihub.kma.go.kr");
...
Run: `cargo test --bin survey test_extract_domain`
Expected: PASS
...
```bash
git add src/bin/survey.rs
git commit -m "feat(survey): extract_domain 헬퍼"
```

---

## Task 4: `survey_single_api()` 비동기 함수

개별 API 페이지를 가져와 `analyze_page()`로 분석하고 `ApiSurveyEntry`를 반환한다.
...
```rust
use korea_cli::core::types::SpecStatus;
...
async fn survey_single_api(
    ...
    // classify()는 has_spec=false, is_skeleton=false로 호출 (조사 단계에서는 스펙 없음)
    // swagger가 있으면 classify에 반영
    let has_spec = analysis.as_ref()
        .map(|a| a.swagger_ops_count.unwrap_or(0) > 0)
        .unwrap_or(false);
    let is_skeleton = analysis.as_ref()
        .map(|a| a.has_swagger_json && a.swagger_ops_count == Some(0))
        .unwrap_or(false);
    let current_classification = format!(
        "{:?}",
        SpecStatus::classify(has_spec, is_skeleton, endpoint_url)
    );
...
이 함수는 HTTP를 호출하므로 단위 테스트 대신 실제 실행으로 검증한다.

**Step 2: 컴파일 확인**

Run: `cargo check --bin survey`
Expected: OK (경고 허용)
...
```bash
git add src/bin/survey.rs
git commit -m "feat(survey): survey_single_api 비동기 페이지 조사"
```

---

## Task 5: `main()` — 카탈로그 수집 + 병렬 조사 + 보고서 출력

**Files:**
- Modify: `src/bin/survey.rs`
- Modify: `Cargo.toml` (bin 추가)

**Step 1: Cargo.toml에 bin 엔트리 추가**

```toml
[[bin]]
name = "survey"
path = "src/bin/survey.rs"
```
...
fn print_summary(report: &SurveyReport) {
    ...
    eprintln!("조사 성공: {} / 실패: {}", report.surveyed_count, report.failed_count);

    // Swagger 분포
    let swagger_full = report.entries.iter()
        .filter(|e| e.swagger_ops_count.unwrap_or(0) > 0)
        .count();
    let swagger_skeleton = report.entries.iter()
        .filter(|e| e.has_swagger_json && e.swagger_ops_count == Some(0))
        .count();
    let swagger_url_only = report.entries.iter()
        .filter(|e| !e.has_swagger_json && e.has_swagger_url)
        .count();
    let no_swagger = report.entries.iter()
        .filter(|e| !e.has_swagger_json && !e.has_swagger_url)
        .count();
    ...
    // HTML 스펙 분포
    let html_available = report.entries.iter()
        .filter(|e| e.has_html_pk && e.html_ops_count > 0)
        .count();
    let html_pk_no_ops = report.entries.iter()
        .filter(|e| e.has_html_pk && e.html_ops_count == 0)
        .count();
    let no_html = report.entries.iter()
        .filter(|e| !e.has_html_pk)
        .count();
    ...
    // 교차 분석: Swagger 없는데 HTML로 가능한 것
    let html_fallback_candidates = report.entries.iter()
...
```bash
git add src/bin/survey.rs Cargo.toml
git commit -m "feat(survey): main 함수 — 카탈로그 수집 + 병렬 조사 + 보고서"
```

---

## Task 6: 이어하기(resume) 기능

12K API 전수조사는 시간이 오래 걸리므로, 중간에 중단되어도 이어할 수 있어야 한다.
...
main() 수정:
- `--resume` 플래그가 있으면 기존 결과를 로드
- 이미 조사된 list_id는 건너뜀
- 새로 조사한 결과와 기존 결과를 병합하여 저장
...
// stream::iter에서 이미 조사된 것 필터:
let services_to_survey: Vec<_> = services.iter()
    .filter(|svc| !existing.contains_key(&svc.list_id))
    .collect();

eprintln!("  신규 조사 대상: {} / 기존: {}", services_to_survey.len(), existing.len());

// ... 조사 실행 (services_to_survey 대상) ...
...
```bash
git add src/bin/survey.rs
git commit -m "feat(survey): --resume 이어하기 기능"
```

---

## Task 7: 빌드 검증 + clippy + fmt

**Step 1: 전체 검증**

```bash
cargo fmt -- --check
cargo clippy --bin survey -- -D warnings
cargo test --bin survey
```

Expected: 모든 체크 통과

**Step 2: 수정 필요 시 수정 후 커밋**

```bash
git add src/bin/survey.rs
git commit -m "chore(survey): clippy + fmt 정리"
```
...
```bash
# 전체 조사 (약 30-40분, concurrency=5 기준)
cargo run --bin survey -- --api-key $DATA_GO_KR_API_KEY

# 빠른 조사 (동시 10건)
cargo run --bin survey -- --api-key $DATA_GO_KR_API_KEY --concurrency 10 --delay 50

# 중단 후 이어하기
cargo run --bin survey -- --api-key $DATA_GO_KR_API_KEY --resume

# 결과 파일: data/survey.json (기본)
```

## 보고서에서 확인할 것

조사 완료 후 다음을 분석하여 번들 빌드 전략을 결정한다:

1. **Swagger 없음 + HTML 폴백 가능** — 이 숫자가 HTML 폴백 통합의 가치를 결정
2. **Anomaly 분포** — 우리가 모르던 패턴이 있는지
3. **도메인 분포** — External로 분류되는 도메인들의 실제 목록
4. **Skeleton + HTML** — Skeleton인데 HTML로 operation이 나오는 경우가 있는지
